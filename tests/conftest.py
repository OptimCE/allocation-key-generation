...
# ---------------------------------------------------------------------------
# Algorithm registry — populated by main.py's lifespan in production, but
# ASGITransport does NOT trigger lifespan, so the registry would be empty
# under tests. Call autodiscover() once per session, idempotently.
# ---------------------------------------------------------------------------
...
def _register_algorithms():
    ...
    # autodiscover raises ValueError on duplicate metadata; guard against
